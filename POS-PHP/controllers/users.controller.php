...
class ControllerUsers
{
	...
	public function ctrAddingDailySales() {
    $today = date('Y-m-d');
    $stmt = Connection::connect()->prepare("SELECT SUM(payments.total) as total 
                                          FROM payments 
		...
		if (isset($_POST["newUser"])) {
			...
			if (
                ...
                swal({
					...
					echo "Database error: ";
					var_dump($answer); // Show DB error info
				}
			} else {
				echo '<script>
            swal({
